    ...
    def ray_cast(self):
        ...
        for ray in range(NUM_RAYS):
            ...
            for i in range(MAX_DEPTH):
                chunk_hor = int(x_hor), int(y_hor)
                if chunk_hor in self.game.map.world_map:
                    texture_hor = self.game.map.world_map[chunk_hor]
                    break
                x_hor += dx
                y_hor += dy
            ...
            for i in range(MAX_DEPTH):
                chunk_vert = int(x_vert), int(y_vert)
                if chunk_vert in self.game.map.world_map:
                    texture_vert = self.game.map.world_map[chunk_vert]
                    break
                x_vert += dx
                y_vert += dy
                depth_vert += delta_depth

            # ray depth, texture offset
